#!/usr/bin/env bash
# setup_env_cuda.sh — clone the `ssp-cuda` environment from the frozen YAML.
#
# Usage:
...
#
# The single source of truth for the env is `ssp_cuda.yaml` next to this
# script, exported with --no-builds from a known-good GPU machine. The
# script auto-detects mamba, falls back to micromamba if the real mamba
# binary is not on PATH, and honours ${MAMBA_EXE} if set. Targets CUDA
# 12.4 (compatible with driver 550+).
